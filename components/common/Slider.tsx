...
const Slider = () => {
  const displayedCard =
    "bg-duo-orange text-white w-full p-6 text-center border-0 rounded-lg";
    ...
    <div className="bg-duo-orange  flex  flex-col-reverse lg:flex-row p-10 border rounded-lg  w-full  h-[430px] sm:h-[350px] lg:h-[250px] relative">
      <div className="w-full lg:w-[750px] lg:h-[230px] absolute top-0 left-0 lg:left-20">
        {testimonials.map((item) => {
          ...
          return (
            <div
              className={counter === index ? displayedCard : "hidden"}
              key={index}
            >
      ...
      <div className="w-full flex justify-evenly lg:justify-between">
        <button onClick={() => counterDown()}>
          <Image src={leftArrow} alt={"Left arrow"} height={30} />
        </button>

        <button onClick={() => counterUp()}>
          <Image src={rightArrow} alt={"Right arrow"} height={30} />
        </button>
      </div>
    </div>
  );
